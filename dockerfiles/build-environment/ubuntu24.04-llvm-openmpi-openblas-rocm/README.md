...
This Docker configuration builds ABINIT with MPI support and optimized OpenBLAS libraries on Ubuntu 24.04, using LLVM AOMP 23.0, OpenMPI 4 and ROCm 7.2.4.

## Configuration Details

**Build Type**: MPI + OpenBLAS + CUDA (parallel,GPU)
**Base Image**: Ubuntu 24.04
**Compilers**: LLVM AOMP 23.0 with MPI wrappers (mpif90, mpicc, mpic++)
**MPI Implementation**: OpenMPI 4 (default) or MPICH
**Parallelization**: MPI + OpenMP hybrid, GPU (ROCm)
...
```bash
docker build \
  -f dockerfiles/build-environment/ubuntu24.04-llvm-openmpi-openblas-rocm/Dockerfile \
  -t abidocker/abinit:ubuntu24-mpi-openblas-rocm \
  .
```
...
docker build \
  ...
  -t abidocker/abinit:ubuntu24-mpi-openblas-rocm-notests \
  .
```
...
docker run --rm \
  -v $(pwd):/workspace \
  abidocker/abinit:ubuntu24-mpi-openblas-rocm \
...
```bash
docker run --rm abidocker/abinit:ubuntu24-mpi-openblas-rocm abinit --version
docker run --rm abidocker/abinit:ubuntu24-mpi-openblas-rocm mpirun --version
```
